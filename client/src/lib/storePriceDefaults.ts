...
/**
 * Quy ước:
 * - defaultCostPrice: giá mua gần nhất.
 * - averageCostPrice: giá mua/giá vốn trung bình.
 * - currentSalePrice: giá bán đang hiệu lực.
 * - defaultSalePrice: giá bán mặc định cũ.
 */
export function getPurchasePriceReference(
      product: any,
): StorePriceReference {
      ...
      if (averagePurchase > 0) {
            return {
                  ...
                  note: "Chưa có giá mua gần nhất · dùng giá mua trung bình",
                  isPurchaseFallback: false,
            };
      }
...
export function getSalePriceReference(
      product: any,
): StorePriceReference {
      const salePrice =
            positive(product?.currentSalePrice) ||
            positive(product?.defaultSalePrice);
      ...
      if (purchase.value > 0) {
            return {
                  ...
                  note:
                        purchase.source === "latest_purchase"
