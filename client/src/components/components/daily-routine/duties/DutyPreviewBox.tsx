...
export function DutyPreviewBox({
      ...
      if (!preview) {
            return (
                  <div className="rounded-2xl border border-amber-100 bg-white/75 p-4 text-sm text-slate-600">
                        Chọn công tác và đối tượng để xem trước các phân công sẽ tạo.
                  </div>
            );
      }
            ...
            <div className={['rounded-2xl border p-4 text-sm', boxClass].join(' ')}>
                  <div className="flex flex-wrap items-center justify-between gap-2">
                        <p className="font-semibold">
                              Sẽ tạo {preview.canCreateCount} phân công
                              {preview.skippedCount > 0
                                    ? `, bỏ qua ${preview.skippedCount} dòng`
                                    : ''}
                        </p>
