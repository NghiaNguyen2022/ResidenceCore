import { Printer, RotateCcw, Save, Settings2, X } from "lucide-react";
                  ...
                  <style>
                        {`
                              @media print {
                                    ...
                                    #finance-voucher-print-area, #finance-voucher-print-area * { visibility: visible !important; }
                                    ...
                                    .finance-voucher-no-print { display: none !important; }
                              }
                        `}
                  </style>
                  ...
                  <div className="flex max-h-[92vh] w-full max-w-5xl flex-col overflow-hidden rounded-[28px] border border-amber-100 bg-white shadow-[0_28px_90px_rgba(15,23,42,0.24)]">
                        <div className="finance-voucher-no-print flex items-center justify-between border-b border-slate-100 px-5 py-4">
                              ...
                              <div className="flex items-center gap-2">
                                    <button type="button" onClick={() => setSettingsOpen((value) => !value)} className="inline-flex items-center rounded-2xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-700 shadow-sm transition hover:-translate-y-0.5 hover:border-amber-200">
                                          <Settings2 className="mr-2 h-4 w-4" /> Cấu hình
                                    </button>
                                    <button type="button" onClick={() => window.print()} className="inline-flex items-center rounded-2xl border border-amber-200 bg-[linear-gradient(135deg,#fff6d8_0%,#f2c866_100%)] px-4 py-2.5 text-sm font-bold text-[#4a2b00] shadow-sm transition hover:-translate-y-0.5">
                                          <Printer className="mr-2 h-4 w-4" /> In phiếu
                                    </button>
                                    <button type="button" onClick={onClose} className="inline-flex h-10 w-10 items-center justify-center rounded-2xl border border-slate-200 bg-white text-slate-600 shadow-sm transition hover:bg-slate-50" aria-label="Đóng">
                                          <X className="h-5 w-5" />
                                    </button>
                              </div>
                        </div>
                        ...
                        <div className="grid min-h-0 flex-1 overflow-hidden lg:grid-cols-[1fr_360px]">
                              <div className="overflow-y-auto bg-slate-100/70 p-4">
                                    <div id="finance-voucher-print-area" className="mx-auto min-h-[720px] max-w-[794px] bg-white px-10 py-9 text-slate-950 shadow-xl">
                                                      ...
                                                      <div>
                                                            ...
                                                            {currentSettings.phone || currentSettings.email ? (
                                                                  <p className="mt-1 text-xs text-slate-500">
                                                                        {currentSettings.phone ? `ĐT: ${currentSettings.phone}` : ""}
                                                                        {currentSettings.phone && currentSettings.email ? " · " : ""}
                                                      ...
                                                      <tbody>
                                                            {rows.map((row, index) => (
                                                                  <tr key={`${row.label}-${index}`}>
                                                                        <td className="border border-slate-300 px-3 py-2"><span className="font-semibold">{row.label}:</span> {row.value}</td>
                                          ...
                                          <footer className="mt-12 border-t border-slate-200 pt-3 text-center text-xs text-slate-500">{currentSettings.footerText}</footer>
                                    </div>
                              </div>
                              ...
                              {settingsOpen ? (
                                    <aside className="finance-voucher-no-print min-h-0 overflow-y-auto border-l border-slate-100 bg-white p-4">
                                          <div className="flex items-center justify-between gap-3">
                                                ...
                                                <button type="button" onClick={() => setDraftSettings(defaultFinanceVoucherSettings)} className="inline-flex items-center rounded-2xl border border-slate-200 px-3 py-2 text-xs font-bold text-slate-600">
                                                      <RotateCcw className="mr-1.5 h-3.5 w-3.5" /> Mặc định
                                                </button>
                                          </div>
                                          ...
                                          <div className="sticky bottom-0 mt-4 border-t border-slate-100 bg-white pt-3">
                                                <button type="button" onClick={saveSettings} className="inline-flex w-full items-center justify-center rounded-2xl border border-amber-200 bg-[linear-gradient(135deg,#fff6d8_0%,#f2c866_100%)] px-4 py-3 text-sm font-bold text-[#4a2b00] shadow-sm">
...
function ConfigInput({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
      return (
            <label className="block">
                  ...
                  <input value={value} onChange={(event) => onChange(event.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none transition focus:border-amber-300 focus:ring-2 focus:ring-amber-100" />
            </label>
      );
}
...
function ConfigTextarea({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
      return (
            <label className="block">
                  <span className="text-xs font-semibold text-slate-600">{label}</span>
                  <textarea value={value} onChange={(event) => onChange(event.target.value)} rows={3} className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none transition focus:border-amber-300 focus:ring-2 focus:ring-amber-100" />
            </label>
      );
}
