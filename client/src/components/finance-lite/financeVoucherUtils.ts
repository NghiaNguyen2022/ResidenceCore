...
export function mergeFinanceVoucherSettings(value?: Partial<FinanceVoucherSettings> | null): FinanceVoucherSettings {
      return {
            ...
            ...(value || {}),
            titles: { ...defaultFinanceVoucherSettings.titles, ...(value?.titles || {}) },
            prefixes: { ...defaultFinanceVoucherSettings.prefixes, ...(value?.prefixes || {}) },
      };
}
...
export function loadFinanceVoucherSettings(): FinanceVoucherSettings {
      ...
      try {
            ...
            return raw ? mergeFinanceVoucherSettings(JSON.parse(raw)) : defaultFinanceVoucherSettings;
      } catch {
...
export function getFinanceVoucherNo(transaction: any, settings = defaultFinanceVoucherSettings) {
      ...
      const id = String(transaction?.id || transaction?.key || "000000").replace(/\D/g, "").slice(-6).padStart(6, "0");
...
export function getFinanceVoucherContent(transaction: any) {
      ...
      return getTransactionSourceMeta(source).label || "Nghiệp vụ tài chính";
}
      ...
      return [
            {
                  label:
                        source === "student_fee_payment"
                              ? "Khoản thu"
                              : isPlannedVoucher(transaction)
                                    ? "Khoản đề nghị"
                                    : isAdvanceVoucher(transaction)
                                          ? "Khoản tạm ứng"
                                          : direction === "out"
                                                ? "Khoản chi"
                                                : "Khoản thu",
