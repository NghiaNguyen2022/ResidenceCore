...
export const skillsRouter = router({
      list: protectedProcedure.query(({ ctx }) => { requireManager(ctx.user); return listSkills(); }),
      create: protectedProcedure.input(payload).mutation(({ ctx, input }) => { requireManager(ctx.user); return createSkill(input); }),
      update: protectedProcedure.input(payload.and(z.object({ id: z.number().int().positive() }))).mutation(({ ctx, input }) => {
