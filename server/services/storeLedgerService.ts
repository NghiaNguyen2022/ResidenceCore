...
export const storeLedgerService = {
      ...
      async confirmDailyClosing(id: number, confirmedBy?: number | null) {
            ...
            if (Boolean((closing as any).postedToFinance)) {
                  ...
                  const existingClosingDate = normalizeDateYmd((closing as any).closingDate, "Ngày chốt sổ không hợp lệ.");
                  const existingClosingCode = String((closing as any).closingCode || `STORE-CLOSING-${id}`);
                  await Promise.all([
                        financeDb.updateFinanceTransactionDescriptionByExternalRef(
                              `${existingBatchId}:IN`,
