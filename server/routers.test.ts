...
export async function getDutyTemplates() {
  try {
    const templates = await db.select().from(dutyTemplates).where(eq(dutyTemplates.isActive, true));
    return templates;
  } catch (error) {
...
export async function getDutyTemplateByCode(templateCode: string) {
  try {
    const template = await db
      .select()
      .from(dutyTemplates)
      .where(and(eq(dutyTemplates.templateCode, templateCode), eq(dutyTemplates.isActive, true)))
      .limit(1);
