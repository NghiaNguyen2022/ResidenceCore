...
import { users, sessions, schools, programs } from "./core";
import {
      residents,
      parents,
...
export const dutyTemplates = mysqlTable("dutyTemplates", {
      ...
      description: text("description"),
      dutyType: mysqlEnum("dutyType", ["daily", "weekly", "monthly", "event"]).notNull(),
...
export const dutyConfigs = mysqlTable("dutyConfigs", {
      ...
      minPersons: int("minPersons").default(1).notNull(),
      maxPersons: int("maxPersons").default(5).notNull(),
      frequency: mysqlEnum("frequency", ["daily", "weekly", "monthly", "event"]).notNull(),
      ...
      weeklyDaysJson: json("weeklyDaysJson"),
      monthWeeksJson: json("monthWeeksJson"),
      monthWeekDaysJson: json("monthWeekDaysJson"),
      monthDaysJson: json("monthDaysJson"),
...
export const dutyChecklists = mysqlTable("dutyChecklists", {
      ...
      description: text("description"),
      stageType: mysqlEnum("stageType", ["normal", "preparation", "during", "after"]).default("normal").notNull(),
      minPersons: int("minPersons").default(1).notNull(),
...
export const dutyAssignments = mysqlTable("dutyAssignments", {
      id: int("id").autoincrement().primaryKey(),
      dutyConfigId: int("dutyConfigId").notNull().references(() => dutyConfigs.id, { onDelete: "cascade" }),
      dutyTaskId: int("dutyTaskId").references(() => dutyChecklists.id, { onDelete: "set null" }),
      residentId: int("residentId").references(() => residents.id, { onDelete: "cascade" }),
